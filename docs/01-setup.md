...
| モデル | 必要メモリ | 備考 |
|--------|-----------|------|
| qwen3:1.7b | 約 2GB | デフォルト。軽量で多くの環境で動作 |
| qwen3:8b | 約 5GB | バランス型 |
| qwen3:14b | 約 9GB | 高品質な回答 |

より高品質な回答が欲しい場合は、メモリに余裕があれば大きいモデルに切り替えられます:

```bash
ollama pull qwen3:14b
OLLAMA_MODEL=qwen3:14b USE_OLLAMA=1 PYTHONPATH=src uv run python -m client.main
```
...
| `OLLAMA_MODEL` | `qwen3:1.7b` | 使用する LLM モデル |
