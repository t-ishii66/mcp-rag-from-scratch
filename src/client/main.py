...
def debug_jsonrpc(direction: str, method: str, data: dict) -> None:
    """JSON-RPCの通信内容を整形表示する（DEBUGモード時のみ）"""
    if not DEBUG:
        return

    # 長いテキストを切り詰めるために、データを深くコピーして加工
...
async def run_client():
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            # サーバー初期化
            debug_jsonrpc("→ 送信", "initialize", {
                "method": "initialize",
                "params": {"capabilities": {}},
            })
            init_result = await session.initialize()
            debug_jsonrpc("← 受信", "initialize", {
                "serverInfo": {"name": init_result.serverInfo.name},
                "capabilities": str(init_result.capabilities),
            })
            print("[1] MCPサーバーに接続しました")

            # ツール一覧を取得（MCP の tools/list）
            debug_jsonrpc("→ 送信", "tools/list", {
                "method": "tools/list",
            })
            tools_result = await session.list_tools()
            mcp_tools = tools_result.tools
            debug_jsonrpc("← 受信", "tools/list", {
                "tools": [
            ...
            while True:
                ...
                if tool_calls:
                    ...
                    # MCPサーバーのツールを呼び出す
                    debug_jsonrpc("→ 送信", "tools/call", {
                        "method": "tools/call",
                        "params": {"name": func_name, "arguments": func_args},
                    })
                    result = await session.call_tool(func_name, func_args)

                    # 検索結果を取得
                    search_result = result.content[0].text if result.content else ""
                    debug_jsonrpc("← 受信", "tools/call", {
