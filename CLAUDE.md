...
- **埋め込み**: `intfloat/multilingual-e5-small`
- **パッケージ管理**: uv
...
2. テキストをチャンクに分割（400文字、80文字オーバーラップ）
...
```bash
RAG_DEBUG=1 uv run python -m client.main
```
...
def test_chromadb_search():
    ...
    # 2. データを入れる
    collection.add(ids=["1"], documents=["大堀翔は野球選手"])

    # 3. 検索する
    results = collection.query(query_texts=["野球"], n_results=1)

    # 4. 結果を確認
    assert "大堀" in results["documents"][0][0]
```
